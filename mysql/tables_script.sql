...
(
    ...
    species INT,
    breed INT,
    CONSTRAINT fk_species FOREIGN KEY (species) REFERENCES species(species_id) ON DELETE SET NULL,
    CONSTRAINT fk_breed FOREIGN KEY (breed) REFERENCES breeds(breed_id) ON DELETE SET NULL
);
...
AFTER UPDATE ON medical_appointment
FOR EACH ROW
BEGIN
	IF NEW.is_completed = TRUE THEN
		INSERT INTO medical_records(appointment_description, animal_id, employee_id) VALUES (NEW.appointment_description, NEW.animal_id, NEW.employee_id);
        DELETE FROM medical_appointment WHERE id= NEW.id;
	END IF;
END //
DELIMITER ;
